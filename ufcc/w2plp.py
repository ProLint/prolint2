...
class LPContacts(object):
    def __init__(
        ...
        for lipid in PLASMA_LIPIDS.keys():
            ...
            frames_idx = counts[
                (counts["# {}".format(lipid)] != 0) & (counts["ResID"] == self.residue)
            ]["FrameID"]
            if len(frames_idx) == 0:
                ...
                self.contacts[lipid] = [
                    x * timestep for x in list((Counter(self.lipids[lipid]).values()))
                ]
                self.lipids[lipid] = np.unique(
                    list(database.selected.residues[self.lipids[lipid]].resindices + 1)
                )
        self.residue += 1
