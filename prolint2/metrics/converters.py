...
class DefaultToSingleConverter(BaseOutputFormatConverter):
    def convert(self) -> SingleOutputFormat:
        ...
        if not isinstance(self.metric_type, str):
            raise ValueError('The metric type must be an string when using the DefaultToSingleConverter')

        for residue_id, lipid_data in self.output_format.items():
            for lipid_id, metric_data in lipid_data.items():
                single_output_format.store_result(residue_id, lipid_id, self.metric_type, metric_data[metric_class_name])
...
class CustomToSingleConverter(BaseOutputFormatConverter):
    def convert(self) -> SingleOutputFormat:
        ...
        if not isinstance(self.metric_type, int):
            raise ValueError('The metric type must be an integer when using the CustomToSingleConverter')

        for residue_id, lipid_data in self.output_format.items():
            for lipid_id, metric_values in lipid_data.items():
                single_output_format.store_result(residue_id, lipid_id, self.metric_type, metric_values[self.metric_type])
